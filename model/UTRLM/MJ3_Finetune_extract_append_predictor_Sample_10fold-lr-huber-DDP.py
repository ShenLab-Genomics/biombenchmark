...
parser.add_argument('--modelfile', type = str, default = 'ESM2SISS_FS4.1_fiveSpeciesCao_6layers_16heads_128embedsize_4096batchToks_lr1e-05_supervisedweight1.0_structureweight1.0_MLMLossMin_epoch93.pkl')

parser.add_argument('--finetune_modelfile', type = str, default = 'model.pt')
...
device = 'cpu' #torch.device('cuda:{}'.format(device_ids[args.local_rank]))
...
def train_step(dataloader, model, data = None):
...
original_train_data = pd.read_csv(f'/data/MRL_Random50Nuc_SynthesisLibrary_Sample/{original_train_file}').iloc[:1000]

train_data = pd.read_csv(f'/data/MRL_Random50Nuc_SynthesisLibrary_Sample/{args.train_file}')
if args.train_atg: 
...
data = data.sample(frac=1, random_state = seed).iloc[:1000].reset_index(drop=True)
...
e_test = pd.read_csv(f'/data/MRL_Random50Nuc_SynthesisLibrary_Sample/{test_file}').iloc[:100].sample(frac = 1, random_state = seed).reset_index(drop=True)
if args.log2: 
...
for i, (train_index, val_index) in enumerate(KF.split(data)):
    ...
    if args.load_wholemodel: 
        ...
        model.load_state_dict({k.replace('module.', ''):v for k,v in torch.load(args.finetune_modelfile, map_location='cpu').items()}, strict = False)
    else:
        ...
        model.esm2.load_state_dict({k.replace('module.', ''):v for k,v in torch.load(args.modelfile, map_location='cpu').items()}, strict = False)
...
#     hidden_visualization_dict = dict()
    for epoch in trange(args.init_epochs+1, args.init_epochs + args.epochs + 2):
        ...
        metrics_train, loss_train = train_step(train_dataloader, model)
